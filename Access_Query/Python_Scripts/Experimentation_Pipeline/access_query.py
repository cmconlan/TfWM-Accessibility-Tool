...
def access_score(trip_results: pd.DataFrame):
    ...
    trip_results['generalised_cost'] = (
                                               1.5 * (trip_results.total_time) -
                                               (0.5 * trip_results.transit_time) + (trip_results.fare * 3600 / 6.7) +
                                               (10 * trip_results.num_transfers)) / 60
    return trip_results.groupby(by=['oa_id', 'poi_type']).generalised_cost.sum(),trip_results.groupby(by=['oa_id', 'poi_type']).generalised_cost.mean()


if __name__ == "__main__":
    #Define directories
    base_folder = '/dcs/project/transport-access-tool/TfWM-Accessibility-Tool/Access_Query/'
    parameter_dir = base_folder + 'Data/Parameters/'
    training_dir = base_folder + 'Data/Training_Data/'
    trips_dir = base_folder + 'Data/Trips/'
    raw_dir = base_folder + 'Data/Raw_Data/'
    ...
    num_trips = query_parameters['gen-trips']['num_trips']
    gen_trips = query_parameters['gen-trips']['gen_trips']
    ...
    if gen_trips == 'Y':
        ...
        trips = generate_trips(oa_ids, poi_types, start_interval, end_interval, num_trips)

        print(f"{len(trips)} trips created.")
        print(f"Sending trips to OTP...")

    else:
        trips = pickle.load(open(trips_dir + "trips.p", "rb"))
        print('Trips Loaded')
        print(f"Sending trips to OTP...")

    start_time = time.time()
    # REMOVE RESPONSES
    results, responses = process_trips(otp_url, trips)
    # REMOVE
    import csv

    with open(results_dir + 'responses.csv', 'w', newline='') as myfile:
    ...
    print('Access Cost Total : ' + str(access_score(results_df)[0]))
    print('Access Cost Mean : ' + str(access_score(results_df)[1]))

    #Output Results
    results_df.to_csv(results_dir + 'otp_results.csv')

    #Output trips to use for inference
    with open(trips_dir + 'trips_details.txt', 'w') as f:
        for item in trips:
            f.write("%s\n" % item.to_dict())
    with open(trips_dir + 'trips.txt', 'w') as f:
        for item in trips:
            f.write("%s\n" % item)