...
def test_population_density(client):
    ...
    for subset in demographic_subsets:
        ...
        density = population_density(subset)
    ...
    for metric in access_metrics:
        ...
        for poi_types in poi_types_seq:
            for strata in time_strata_seq:
                ...
                print(query_url)
                metrics = calculate_access_metric(metric, poi_types, strata)
                response = json.loads(client.get(query_url).data)
                # print(response)
                assert contains_all_output_areas(oa_ids, response['oa-level'])
                if not metrics_match_expected(response['oa-level'], metrics):
                    print(query_url)
                    pytest.fail()
